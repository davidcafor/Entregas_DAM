    ...
    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
        ...
        for (Persona p : personas) {
            if (txtDni.getText().equals(p.getDni())) {
                sb.append("- El DNI ya existe en el registro");
                formularioOK = false;
            }
        }
        ...
        if(!txtNombre.getText().matches("[A-Za-zÀ-ÿ '-]*")){
            sb.append("- Nombre contiene caracteres no válidos");
            formularioOK = false;
        }
        ...
        if(!txtApellido1.getText().matches("[A-Za-zÀ-ÿ '-]*")){
            sb.append("- Apellido 1 contiene caracteres no válidos");
            formularioOK = false;
        }
        ...
        if(!txtApellido2.getText().matches("[A-Za-zÀ-ÿ '-]*")){
            sb.append("- Apellido 2 contiene caracteres no válidos");
            formularioOK = false;
        }
        if (bmSexo == null) {
            sb.append("- Falta seleccionar Sexo\n");
            formularioOK = false;
        }
        if (bmTitulacion == null) {
            sb.append("- Falta seleccionar titulacion\n");
            formularioOK = false;
        }
        ...
        if (rdoOtra.isSelected()) {
            if (txtOtraTitulacion.getText().trim().isBlank()) {
                sb.append("- Debes rellenar el cuadro Otra\n");
            } else {
                rdoOtra.setActionCommand(txtOtraTitulacion.getText().trim());
            }
        }

        //COMPROBACION DE LA VALIDACION
        if (formularioOK) {
